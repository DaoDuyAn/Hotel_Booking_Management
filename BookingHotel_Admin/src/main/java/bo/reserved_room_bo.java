...
public class reserved_room_bo {
	...
	public boolean isEveryReservationFullyPaid(ArrayList<reserved_room_bean> lst) {
		for (reserved_room_bean rdbean : lst) {
			if (rdbean.getStatus().equals("Pending approval") || rdbean.getStatus().equals("Cancellation pending")) {
				return false;
			}
		}
	...
	public ArrayList<reserved_room_bean> getReservedConfirm(String status) throws Exception {
		return rdao.getReservedConfirm(status);
	}

	public int updateStatusByRdId(int reserved_room_id, String status) throws Exception {
		return rdao.updateStatusByRdId(reserved_room_id, status);
	}
	
	public String statistic_2() throws Exception {
		return rdao.statistic_2();
	}
}
