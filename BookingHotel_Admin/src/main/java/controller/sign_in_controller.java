		...
		try {
			...
			if (txtcapcha != null) {
				if (!captcha.isCorrect(txtcapcha)) {
					request.setAttribute("wcapcha", "Incorrect Captcha, please try again");
			...
			if (un != null && pass != null) {
				...
				int user_id = accountbo.checkLogin(un, pass, (int) 2);
				if (user_id != 0) {
					...
					account_bean accountbean = accountbo.getAccount(un, (int) 2);
					...
					if (accountbean != null) {
						...
						request.setAttribute("wuser", "Username does not exist");
					}
